...
def get_config():
    ...
    return {
        ...
        "nginx_domain":      "",
        "nginx_dir":         "",
        "nginx_upstream":    "remnawave-backup-manager:8090",
    }
...
def render_page(backups, cfg, log):
    ...
    for h in cfg.get("remote_hosts", []):
        hosts_html += f'''<div class="host-item">
            ...
            <div class="host-row-3">
                ...
                <input class="host-dir" placeholder="/root/remnawave-backups" value="{h.get('dir','')}">
            </div>
        </div>'''
    ...
    html = html.replace("{{NGINX_DOMAIN}}",      cfg.get("nginx_domain", ""))
    html = html.replace("{{NGINX_DIR}}",         cfg.get("nginx_dir", ""))
    html = html.replace("{{NGINX_UPSTREAM}}",    cfg.get("nginx_upstream", "remnawave-backup-manager:8090"))
    return html
...
def find_nginx_dir():
    for p in NGINX_CANDIDATES:
        if os.path.isfile(f"{p}/nginx.conf") and os.path.isfile(f"{p}/docker-compose.yml"):
            return p
    return None

@app.get("/api/nginx/detect")
async def nginx_detect(user=Depends(check_auth)):
    """Автоопределение папки nginx."""
    p = find_nginx_dir()
    if p:
...
async def nginx_apply(request: Request, user=Depends(check_auth)):
    """Добавить server block в nginx.conf, тома сертификатов в docker-compose.yml и перезапустить nginx."""
    body      = await request.json()
    domain    = body.get("domain", "").strip()
    nginx_dir = body.get("nginx_dir", "").strip()
    upstream  = body.get("upstream", "remnawave-backup-manager:8090").strip()
    if not upstream:
        upstream = "remnawave-backup-manager:8090"
    # Пути сертификатов фиксированы — всегда внутри контейнера
    cert_file = "/app/ssl/fullchain.pem"
    key_file  = "/app/ssl/privkey.key"

    # Очищаем домен от протокола если пользователь вставил полный URL
    domain = domain.replace("https://", "").replace("http://", "").rstrip("/")

    if not domain:    raise HTTPException(400, "Не указан домен")
    if not nginx_dir: raise HTTPException(400, "Не указан путь к nginx")

    nginx_conf    = f"{nginx_dir}/nginx.conf"
    nginx_compose = f"{nginx_dir}/docker-compose.yml"

    if not os.path.isfile(nginx_conf):
        raise HTTPException(400, f"nginx.conf не найден в {nginx_dir}")
    if not os.path.isfile(nginx_compose):
        raise HTTPException(400, f"docker-compose.yml не найден в {nginx_dir}")

    # Проверяем что сертификаты существуют
    if not os.path.isfile(cert_file):
        raise HTTPException(400, f"Сертификат не найден: {cert_file}. Выпустите SSL сертификат через install.sh")
    if not os.path.isfile(key_file):
        raise HTTPException(400, f"Ключ не найден: {key_file}. Выпустите SSL сертификат через install.sh")

    # nginx.conf — добавить server block если домена ещё нет
    with open(nginx_conf) as f:
        conf_content = f.read()

    if domain not in conf_content:
        server_block = f"""
server {{
    server_name {domain};
    listen 443 ssl;
...
"""
    ...
    # docker-compose.yml — добавить тома сертификатов если ещё нет
    with open(nginx_compose) as f:
        compose_content = f.read()

    if "backup.fullchain.pem" not in compose_content:
        import re
        new_volumes = (
            f"      - {cert_file}:/etc/nginx/ssl/backup.fullchain.pem:ro\n"
            f"      - {key_file}:/etc/nginx/ssl/backup.privkey.key:ro\n"
        )
        pattern = r"(volumes:(?:\n\s+- [^\n]+)+)"
        match = re.search(r"(volumes:(?:\n      - [^\n]+)+)", compose_content)
        if match:
            insert_pos = match.end()
            compose_content = compose_content[:insert_pos] + f"\n      - {cert_file}:/etc/nginx/ssl/backup.fullchain.pem:ro\n      - {key_file}:/etc/nginx/ssl/backup.privkey.key:ro" + compose_content[insert_pos:]
            with open(nginx_compose, "w") as f:
                f.write(compose_content)

    # Перезагрузка nginx без остановки (nginx -s reload)
    # Сначала находим имя контейнера nginx
    result_ps = subprocess.run(
        ["docker", "compose", "ps", "--format", "{{.Name}}"],
        cwd=nginx_dir, capture_output=True, text=True
    )
    nginx_container = next(
        (l.strip() for l in result_ps.stdout.splitlines() if "nginx" in l.lower()),
        None
    )
    ...
    if nginx_container:
        ...
        if result2.returncode != 0:
            raise HTTPException(500, f"Ошибка reload nginx: {result2.stderr[:200]}")
    else:
        # Fallback — перезапуск через compose если контейнер не найден
        subprocess.run(["docker", "compose", "up", "-d"], cwd=nginx_dir,
                      capture_output=True, text=True)

    # Сохраняем nginx настройки в config для восстановления в UI
    cfg = get_config()
    cfg["nginx_domain"]   = domain
    cfg["nginx_dir"]      = nginx_dir
    cfg["nginx_upstream"] = upstream
    save_config(cfg)

    return {"status": "ok", "message": f"nginx настроен для {domain} и перезагружен"}

@app.get("/api/nginx/status")
async def nginx_status(user=Depends(check_auth)):
    """Проверить статус nginx."""
    nginx_dir = find_nginx_dir()
    if not nginx_dir:
        # Пробуем взять из сохранённого конфига
        cfg = get_config()
        nginx_dir = cfg.get("nginx_dir", "")
    if not nginx_dir:
        raise HTTPException(404, "nginx не найден. Укажите путь вручную.")

    # Запущенные контейнеры nginx
    result = subprocess.run(
        ["docker", "compose", "ps", "--format", "{{.Name}}"],
        cwd=nginx_dir, capture_output=True, text=True
    )
    ...
    # Проверяем домен backup в nginx.conf
    nginx_conf = f"{nginx_dir}/nginx.conf"
    domain_found = False
    domain = ""
    if os.path.isfile(nginx_conf):
        with open(nginx_conf) as f:
            conf = f.read()
        import re
        match = re.search(r"server_name\s+(backup\.[^\s;]+)", conf)
        if match:
            domain_found = True
